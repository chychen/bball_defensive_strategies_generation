...
class DataFactory(object):
    ...
    def normalize_offense(self, input_):
        """ normalize player x,y,z on input
        input_ : shape=[None, length, 13]
        """
        # x
        input_[:, :, [0, 3, 5, 7, 9, 11]] = (
            input_[:, :, [0, 3, 5, 7, 9, 11]] - self.__norm_dict['x']['mean']) / self.__norm_dict['x']['stddev']
        # y
        input_[:, :, [1, 4, 6, 8, 10, 12]] = (
            input_[:, :, [1, 4, 6, 8, 10, 12]] - self.__norm_dict['y']['mean']) / self.__norm_dict['y']['stddev']
        # z
        input_[:, :, 2:3] = (input_[:, :, 2:3] - self.__norm_dict['z']
                             ['mean']) / self.__norm_dict['z']['stddev']
        return input_
